        ...
        private void mSearch_TextChanged(object sender, TextChangedEventArgs e)
        {
            if (mTestCase3s != null)
            {
                List<testCase3> searchedDriver = (from userinfo in mTestCase3s
                    where
                        userinfo.lianxidianhua.Contains(mSearch.Text, StringComparison.OrdinalIgnoreCase) ||
                        userinfo.sijiname.Contains(mSearch.Text, StringComparison.OrdinalIgnoreCase)
